# -*- coding: utf-8 -*-
"""Phi-3.5 Instruct Unsloth 2x faster finetuning.ipynb
...
#use this to generated user queries
# data = pd.read_csv("train.csv")
# data["user"] = data["user"].apply(func)
...
messages = [
    {"from": "human", "value": "Return only a numeric value between 0 to 5, with 0 being no priority and 5 \
    being criminal prioirty of the complaint:\
    hanging chains of the middle berth broken  pnr 4512791357"},
]
...
outputs = model.generate(input_ids = inputs, max_new_tokens = 2, use_cache = True)
tokenizer.batch_decode(outputs)

out = tokenizer.batch_decode(outputs)
import re
print(type(out))
re.findall(r'\d+',out[0].split(">")[-1])
...
def inference(inp):
    ...
    import re
    print(type(out))
    return re.findall(r'\d+',out[0].split(">")[-1])
...
_ = model.generate(input_ids = inputs, streamer = text_streamer, max_new_tokens = 5, use_cache = True)
